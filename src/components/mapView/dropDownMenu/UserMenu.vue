      ...
      >
        <div class="py-1">
          <MenuItem v-slot="{ active }">
            <button
              @click="handleOpenTrip"
              :class="[
                active ? 'bg-red-custom text-beige-custom' : 'text-red-custom',
                'block px-4 py-2 text-left text-sm font-semibold w-full h-full'
              ]"
            >
              Mes trajets
            </button>
          ...
          <form method="POST" action="#">
            <MenuItem v-slot="{ active }">
              <button
                ...
                :class="[
                  active ? 'bg-red-custom text-beige-custom' : 'text-red-custom',
                  'block w-full px-4 py-2 text-left text-sm font-semibold h-full'
                ]"
              >
                Déconnexion
              </button>
